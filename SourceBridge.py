...
def get_SE_wfm_saved(instrument, wfm_name, wfm_type = None):
    ...
    file.close()
    # This was debug when tracking down some weird behavior with markers.
    # Ended up being a logical error resulting in reading the marker data with an offset. 
    # Keeping this here in case I need to export the csv's again. 
...
def Transfer_Setup(SourceXpress,AFG,Setup):
    ...
    if Ch1.Clock == 'EXT': # only induce that 150mS delay if using extrnal clocking.  
        time.sleep(0.15) # give the AFG 150mS to lock to an external source before querying if the lock was successful. 
                         # Lock is attempted every 100mS, this should garentee at least 1 lock attempt was completed.  
        if int(AFG.query('STATus:QUEStionable:CONDition?')) != 0:
            log.error('External ref not found, aborting')
            SourceXpress.write('AWGCONTROL:STOP:IMMEDIATE')
            return(-1)
        else: log.info('External ref locked')
    ...
    if Mode == 'WAV':
        ...
        # Enable the Marker (trigger) output
        AFG.write('SEQuence:ELEM1:MARKer:STATe 1')
            ...
            if not self.Error: # connection was made, inntialize the generator
                ...
                if self.inst.ID not in valid_ids:
                    ...
                    if self.firmware_id < 10605:
                        ...
                        self.inst.write('Display:Text "Instrument controlled by SourceBridge"')
        ...
        if Reply_Name not in AFG_Dict:  
            ...
            if Reply_State == AFG_Dict[Reply_Name].Output_State:
                ...
                if system_config != 0: 
                    ...
                    log.info(f'Total processing time: {round(time.time()-timer_start,2)}s')
